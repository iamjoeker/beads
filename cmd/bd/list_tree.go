...
func listFooterLine(total int, byStatus []statusCount, truncated, readyFiltered bool, statusSelector string) string {
	if readyFiltered {
...
func displayPrettyListWithDepsMode(issues []*types.Issue, showHeader bool, allDeps map[string][]*types.Dependency, depsMode string, truncated, readyFiltered bool, statusSelector string) {
	...
	fmt.Println()
	fmt.Println(strings.Repeat("-", 80))
	fmt.Println(listFooterLine(len(issues), countIssuesByStatus(issues), truncated, readyFiltered, statusSelector))
	fmt.Println()
