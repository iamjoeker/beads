...
// test finishes.
//
// Unlike StartIsolatedDoltContainer this does NOT touch any of
// doltPortEnvVars: those are process-wide, so a test that only wants its own
// server should not perturb sibling tests sharing the process.
...
// mapped host port, additionally pointing every var in doltPortEnvVars at it
// for the duration of the test. Publishing all of them (not just
// BEADS_DOLT_PORT) is what keeps an ambient BEADS_DOLT_SERVER_PORT from
// shadowing the container — see doltPortEnvVars (bd-799).
